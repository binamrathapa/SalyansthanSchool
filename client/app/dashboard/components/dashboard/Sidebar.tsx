            ...
            {item.children ? (
              <>
                <button
                  ...
                  className={`flex items-center w-full p-3 rounded-lg transition-all duration-200 ${openAccordion === idx
                ...
                >
                  <div className="flex items-center gap-3">
                    <item.icon
                      className={`h-5 w-5 ${openAccordion === idx
                        ? "text-[var(--brand-600)]"
                        : "text-[var(--text-muted)]"
                        }`}
                    />
                  ...
                  <ul className="mt-1 ml-4 space-y-1">
                    {item.children.map((child) => (
                      <li key={child.name}>
                        <Link
                          href={child.path}
                          className={`flex items-center gap-2 w-full p-2 pl-8 rounded-lg transition-all duration-200 ${isActive(child.path)
              ...
              <Link
                href={item.path || "#"}
                className={`flex items-center w-full p-3 rounded-lg transition-all duration-200 ${isActive(item.path)
                  ? "bg-[var(--brand-50)] text-[var(--brand-700)] border border-[var(--brand-200)]"
                  : "text-[var(--text-default)] hover:bg-[var(--brand-50)] hover:text-[var(--brand-600)]"
